  ...
  public render() {
    ...
    if (this.props.stores.ViewportStore.currentEditInstanceKey) {
      ...
      childs = this.props.actions.ViewportAction.getInstancePath(
        ...
        const instance = (this.props.stores.ViewportStore.instances.get(instanceKey) as InstanceInfo);
        const setting = this.props.actions.ApplicationAction.getSettingByInstance(instance);

        return (
          <Styled.FooterItem
            onClick={this.handleClick.bind(this, instanceKey)}
    ...
    return (
      <Styled.Container>
        <Styled.AutoWidthContainer onMouseLeave={this.handleMouseLeave}>{childs}</Styled.AutoWidthContainer>
      </Styled.Container>
    );
  }
