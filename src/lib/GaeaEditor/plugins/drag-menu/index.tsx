  ...
  public render() {
    return (
      <Styled.Container>
        ...
        <Styled.ListContainer ref={(ref: HTMLDivElement | null) => (this.listContainer = ref)}>
          {this.getList()}
  ...
  private getList = () => {
    return Array.from(this.props.stores.ApplicationStore.componentClasses)
      .filter(([gaeaKey, componentClass]) => {
        const setting = this.props.stores.ApplicationStore.componentSetting.get(gaeaKey) || {key: '', name: ''};
      ...
      .concat(
        Array.from(this.props.stores.ApplicationStore.preComponents).map(([gaeaKey, preComponentInfos], index) => {
          const componentClass = (this.props.stores.ApplicationStore.componentClasses.get(gaeaKey) as any);
