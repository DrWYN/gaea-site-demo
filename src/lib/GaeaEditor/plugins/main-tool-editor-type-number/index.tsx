...
const parseInputToOutRange = (value: number | string | undefined, inputRange: number[], outputRange: number[]) => {
...
class MainToolEditorNumber extends React.Component<Props, State> {
  ...
  public render() {
    ...
    // 当前值转换成 inputValue
    if(this.numberData.outputRange) {
      currentValue = parseInputToOutRange(currentValue, this.numberData.outputRange, this.numberData.inputRange) || 0;
    }
  ...
  private handleChange = (value: number | string | undefined) => {
    // 转换
    if(this.numberData.outputRange) {
      value = parseInputToOutRange(value, this.numberData.inputRange, this.numberData.outputRange) || 0;
    }
