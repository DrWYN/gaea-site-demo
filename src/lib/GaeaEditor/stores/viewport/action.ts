...
export default class ViewportAction {
  ...
  public addInstance(params: {
    ...
    if (params.parentInstanceKey !== null) {
      ...
      if (parentInstance.gaeaKey === 'gaea-container' && params.gaeaKey === 'gaea-container') {
        if (
          this.getInstanceProps(params.parentInstanceKey, 'style.display') === 'flex' &&
  ...
  public getSiblingInstances(instanceKey: string) {
    ...
    return parentInfo.childs.map(childInstanceKey => this.store.instances.get(childInstanceKey));
  }
  ...
  public registerInnerDrag(
    ...
    Sortable.create(dragParentDom, {
        ...
        switch (this.store.currentDragInfo.type) {
          ...
          case 'viewport':
            ...
            if (this.store.currentDragInfo.dragStartParentDom.childNodes.length === 0) {
              ...
              this.store.currentDragInfo.dragStartParentDom.childNodes.length ===
              this.store.currentDragInfo.dragStartIndex
            ) {
      ...
      onRemove: (event: any) => {
        ...
        const dragTargetKey = this.store.instances.get(parentInstanceKey).childs[
          this.store.currentDragInfo.dragStartIndex
        ];
    ...
    Sortable.create(dragParentDom, {
      ...
      onStart: (event: any) => {
        ...
        if (event.item.dataset.source) {
          ...
          if (this.applicationStore.onComponentDragStart) {
